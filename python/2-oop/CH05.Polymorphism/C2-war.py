...
class War(CardGame):
    ...
    def __deal_hand(self):
        dealt = []
        for _ in range(5):
            dealt.append(self.deck.deal_card())
        return dealt
...
class Card:
    ...
    @staticmethod
    def __index_of(lst, item):
        for i in range(len(lst)):
            if lst[i] == item:
                return i
        raise ValueError("No Suit or Rank matches this item")

    def __cmp(self, other):
        self_suit_i = self.__index_of(SUITS, self.suit)
        other_suit_i = self.__index_of(SUITS, other.suit)
        self_rank_i = self.__index_of(RANKS, self.rank)
        other_rank_i = self.__index_of(RANKS, other.rank)
