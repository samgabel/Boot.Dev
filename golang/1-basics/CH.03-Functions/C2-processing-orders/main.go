...
func placeOrder(productID string, quantity int, accountBalance float64) (bool, float64) {
	...
	// checking stock against order quantity
	if availableStock < quantity {
		return false, accountBalance
	}
	// checking total price against accountBalance
	if totalCost > accountBalance {
		return false, accountBalance
	}
	// if all guard clauses pass (we execute the order)
	return true, accountBalance - totalCost
}
