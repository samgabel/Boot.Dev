// We are essentially just grabbing the `Response` object from the `fetch() method: 'GET'` function.
// We then update it with `fetch() method: 'PUT'`.
// Finally we check the change with another `fetch() method: 'GET'`

// note that this is all using the same API key
